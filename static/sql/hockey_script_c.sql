...
CREATE VIEW hockey_view_c AS
  SELECT
    award_player.player_id,
    award_player.year,
    COUNT(award_player.award) AS award_count
  FROM award_player
...
CREATE VIEW player_award_tm AS
  SELECT
    hockey_view_c.*,
    ANY_VALUE(scoring.tm_id) AS tm_id
  FROM hockey_view_c
    LEFT JOIN (scoring)
...
CREATE VIEW player_coach_max AS
  SELECT max_player_award_per_year.*
  FROM max_player_award_per_year
    INNER JOIN max_coach_wins_per_year
      ON max_player_award_per_year.tm_id = max_coach_wins_per_year.tm_id AND
         max_player_award_per_year.year = max_coach_wins_per_year.year;

# Join the master info with the list of players from player_coach_max
SELECT
  master.*,
  player_coach_max.year AS max_year,
  player_coach_max.award_count
FROM master
